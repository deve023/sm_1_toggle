 ...
 */
void TimerService::setRuncycleTimerFor(CycleBasedInterface *statemachine,  sc_integer cycle_period) {
...
void TimerService::proceed(sc_integer elapsed_ms) {
	...
	sc_integer time_to_proceed = std::min(time_till_next_task(), elapsed_ms);
	sc_integer proceeded_time = 0;
	while (time_to_proceed > 0) {
		...
		do {
			...
			sc_integer best_remaining_time = tasks[best].data.get.time_ms - tasks[best].elapsed_time_ms;
			...
			while (next_task < length) {
				...
				sc_integer remaining_time = current_task.data.get.time_ms - current_task.elapsed_time_ms; 
...
sc_integer TimerService::time_till_next_task() {
	if (next_active_task == length) {
		return -1;
	}
	
	sc_integer time = tasks[next_active_task].data.get.time_ms - tasks[next_active_task].elapsed_time_ms;
